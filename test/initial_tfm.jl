...
@testset "initial_tfm improves rigid translational alignment" begin
    ...
    @test mm4 < 1.0e-8 #this mismatch should be lower!
...
@testset "Test initial_tfm creates real rotations." begin #some of this seems redundant
    ...
    tformtest0, mm0 = qd_rigid(testimage2, testimage1, mxshift, mxrot2; fvalue = 1.0e-5, rtol = 0, print_interval = typemax(Int))
    ...
    tformtest8, mm8 = @test_logs (:warn, "initial_tfm is not a rigid transformation") begin
        qd_rigid(testimage6, testimage5, mxshift, mxrot2; SD = SD, print_interval = typemax(Int), initial_tfm = AffineMap(SD \ tformtest5.linear * SD, SD \ tformtest5.translation))
    end
    @test mm8 < 1.0e-4
...
@testset "Test initial_tfm improves translational alignment for affine" begin
    ...
    tformtest4, mm4 = qd_affine(testimage3, testimage4, mxshift; print_interval = typemax(Int), initial_tfm = init_tfm, fvalue = 1.0e-5) #
    @test mm4 < 1.0e-4 #this mismatch should be lower!
...
@testset "Test initial_tfm improves general alignment for affine" begin
    ...
    tformtest2, mm2 = qd_affine(testimage2, testimage1, mxshift; print_interval = typemax(Int), initial_tfm = mytform, fvalue = 1.0e-5) #
    #with the initial_tfm being the true tfm, this should give back the true rotation
