...
@testset "QuadDIRECT tests with random images" begin
    ...
    @test sum(abs.(tfm0.translation - tfm.translation)) < 1.0e-3
    ...
    tfm, mm = qd_rigid(fixed, moving, mxshift, mxrot; SD = SD, thresh = thresh, maxevals = 1000, rtol = 0, fvalue = 1.0e-8)
    tfm = arrayscale(tfm, SD)
    @test sum(abs.(tfm0.linear - tfm.linear)) < 1.0e-3
    ...
    tfm, mm = qd_affine(fixed, moving, mxshift; SD = SD, thresh = thresh, rtol = 0, fvalue = 1.0e-3, maxevals = 10^4)
    tfm = arrayscale(tfm, SD)
